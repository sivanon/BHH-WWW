            ...
            {data.items.length === 0 ? (
              ...
              data.items.map((item, itemIdx) => (
                <React.Fragment key={itemIdx}>
                  <tr className="bg-orange-50/40 border-b border-orange-100/60 hidden md:table-row">
                    <td colSpan={4} className="p-4">
                      <div className="flex items-start">
                        <ChevronRight className="w-5 h-5 text-orange-500 mr-2 shrink-0 mt-0.5" />
                        <div>
                          <span className="font-extrabold text-orange-900 mr-2 tracking-wide">{item.code}</span>
                          <span className="font-medium text-orange-800 leading-relaxed text-[15px]">{item.title}</span>
                        </div>
                      </div>
                    </td>
                  </tr>
                  ...
                  {item.documents.length === 0 ? (
                    <tr className="border-b border-gray-100 bg-white hidden md:table-row">
                      <td colSpan={4} className="p-6 text-center">
                        <span className="text-gray-400 italic text-sm font-medium">- รอการอัปโหลดไฟล์ในหมวดนี้ -</span>
                      </td>
                    </tr>
                  ) : (
